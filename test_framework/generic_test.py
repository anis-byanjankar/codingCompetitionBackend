...
def run_tests(handler, config, res_printer):
    ...
    for test_case in test_data[1:]:
        ...
        results[test_nr] = {
            'result': result,
            'time': round(test_output.timer.get_microseconds(), 2) if test_output.timer else None
        }
    ...
    if durations:
        ...
        evaluation = {
            ...
            'average_time': round(statistics.mean(durations), 2),
            'median_time': round(statistics.median(durations), 2),
